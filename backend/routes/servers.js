...
export function serversRouter() {
  ...
  // DELETE /servers/:name — admin cleanup, protected by the ADMIN secret (NOT the
  // webhook secret handed to server operators). Removes every server row of that name
  // (case-insensitive) and cascades its events/timers/deaths. Mainly to purge orphan
  // rows the webhook captured pre-/setup.
  router.delete('/:name', verifyAdminSecret, (req, res) => {
