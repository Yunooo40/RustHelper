...
test('chaque commande expose { data, execute } et se sérialise (deploy-commands)', async () => {
  ...
  assert.ok(cmds.length >= 18, `attendu >=18 commandes, vu ${cmds.length}`);
...
test('noms de commande uniques + commandes multi-serveur présentes', async () => {
  ...
  for (const n of ['setup', 'servers', 'server-default', 'server-remove', 'status', 'events', 'timer', 'pop', 'time', 'pair', 'unpair', 'notify', 'watch']) {
